      ...
      <div v-for="item in companyList" :key="item.id">
        <mu-list-item>
          ...
          <mu-list-item-content>
            ...
            <mu-list-item-sub-title>累计:{{item.total}} 今天:{{item.day}} 未下载:{{item.dayWei}} 库存:{{item.stock}}
            </mu-list-item-sub-title>
          </mu-list-item-content>
        </mu-list-item>
        <div class="btn">
          <mu-button class="add" :flat="item.stock > 100" :color="item.stock < 100 ? 'red' : ''"
                     :to="'/ranwawa/number?id=' + item.id">添加单号
          </mu-button>
          <mu-button :flat="!item.dayWei" :color="item.dayWei ? 'primary' : 'none'" @click="downloadExcel(item)">下载单号
          </mu-button>
          <mu-button flat @click="clearStock(item, index)">清空库存</mu-button>
        </div>
        <mu-divider></mu-divider>
      </div>
    </mu-list>
    <div class="priceComputed">
      <mu-text-field @input="computedPrice" v-model="price.cost" placeholder="请输入进货价"></mu-text-field>
      <mu-divider></mu-divider>
      成本价：{{price.cost}}
      <mu-divider></mu-divider>
      会员价：{{price.normal}}
...
export default {
  ...
  methods: {
    ...
    downloadExcel(item) {
      ...
      // 发货模板 龙邦是1 其他是0
      param.template = this.templates.find(ele => ele.express_id === (item.id === 51 ? 1 : 0)).template;
      order.downloadExcel(param).then(res => {
        const cb = () => {
          item.dayWei = 0;
  ...
  mounted() {
    order.getCompany().then(res => {
      const cb = () => {
        this.companyList = res.data.data;
        this.companyList.forEach((ele, i) => {
          order.getCompanyDetail({ express_id: ele.id }).then(res => {
            const cb = () => {
